...
interface TechPrepData {
  codingChallenges: TechQA[];
  systemDesign: TechQA[];
  conceptual: TechQA[];
  debugging: TechQA[];
  toolsAndTech: TechQA[];
...
const CATEGORIES = [
  { key: "codingChallenges", label: "Coding", icon: Code2, color: "text-emerald-400 bg-emerald-400/10" },
  { key: "systemDesign", label: "System Design", icon: Cpu, color: "text-blue-400 bg-blue-400/10" },
  { key: "conceptual", label: "Conceptual", icon: Brain, color: "text-violet-400 bg-violet-400/10" },
  { key: "debugging", label: "Debugging", icon: Bug, color: "text-amber-400 bg-amber-400/10" },
  { key: "toolsAndTech", label: "Tools & Tech", icon: Wrench, color: "text-pink-400 bg-pink-400/10" },
...
export default function TechnicalPrepPage() {
  ...
  const [prep, setPrep] = useState<TechPrepData | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [activeTab, setActiveTab] = useState("codingChallenges");
  ...
  const generate = async (title: string, comp: string, sk: string[]) => {
    ...
    try {
      const res = await fetch("/api/interview-prep", {
        ...
        body: JSON.stringify({ jobTitle: title, company: comp, skills: sk, type: "technical" }),
      });
      ...
      <header className="sticky top-0 z-50 border-b border-white/[0.04] bg-background/70 backdrop-blur-2xl backdrop-saturate-150">
        <div className="mx-auto flex h-14 max-w-5xl items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[13px] font-bold tracking-tight">Technical Interview</span>
          </div>
          <div className="w-16" />
        </div>
      </header>
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6">
        ...
        {loading && (
          <div className="mt-16 flex flex-col items-center justify-center">
            <Loader2 size={28} className="mb-3 animate-spin text-emerald-400" />
            <p className="text-sm text-muted-foreground">
              Generating technical questions for {customTitle || jobTitle}...
            </p>
        ...
        {prep && !loading && (
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mt-6">
            ...
            <div className="mt-4 space-y-3">
              {/* Q&A sections */}
              {["codingChallenges", "systemDesign", "conceptual", "debugging", "toolsAndTech"].includes(activeTab) && (
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    className="space-y-3"
                  >
                    {(prep[activeTab as keyof TechPrepData] as TechQA[])?.map((qa, i) => {
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="flex-1">
                              ...
                              {qa.difficulty && (
                                <span className={`ml-2 inline-flex rounded-md px-1.5 py-0.5 text-[10px] font-bold ${DIFFICULTY_STYLES[qa.difficulty] || ""}`}>
                                  {qa.difficulty}
                                </span>
                              )}
                            </div>
        ...
        {!loading && !prep && !error && !jobTitle && (
          <div className="mt-20 text-center">
            <Code2 size={36} className="mx-auto mb-3 text-muted-foreground" />
            <p className="text-muted-foreground">
              Enter a job title above to generate technical interview questions.
            </p>
          </div>
        )}
      </div>
